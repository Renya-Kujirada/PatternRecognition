...
//2.1訓練サンプル、テストサンプルをそれぞれ分割する
 ...
 // クラス50個のサンプルのうち、奇数番を訓練サンプル、偶数番をテストサンプルとする
void holdout(double d[M][N],double k[M][NN],double t[M][NN]){
	int i,j,kc = 0,tc = 0;
	for(i=0;i<M;i++){
		for(j=0;j<N;j++){
			if( ((j+1) % 2)  == 1 ){	// j+1 が実際のサンプル番号なので、j+1を2で割った余りが1、
				k[i][kc++]=d[i][j];	//すなわち、奇数番の場合、訓練サンプルとしている。
			}
			else{
				t[i][tc++]=d[i][j];	// 偶数番25個をテストサンプル用配列に格納
			}
		}
		kc = 0, tc = 0; // カウンタの初期化
	}
...
int main(void){
  ...
  //*****************************************************************************************//
	...
	cal_vector(training_setosa,se_avector);	 		//setosaの奇数番25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_setosa,se_avector,setosa_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(setosa_CM,setosa_CMI);

	cal_vector(training_versic,ve_avector);	 		//versicolorの奇数番25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_versic,ve_avector,versic_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(versic_CM,versic_CMI);

	cal_vector(training_virgin,vi_avector);	 		//verginicaの奇数番25個のデータを訓練サンプルとして平均ベクトルを求める
