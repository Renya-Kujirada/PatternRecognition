...
//2.1訓練サンプル、テストサンプルをそれぞれ分割する
 ...
 // クラス50個のサンプルのうち、偶数番を訓練サンプル、奇数番をテストサンプルとする
void holdout(double d[M][N],double k[M][NN],double t[M][NN]){
	int i,j,kc = 0,tc = 0;
	for(i=0;i<M;i++){
		for(j=0;j<N;j++){
			if( ((j+1) % 2)  == 0 ){	// j+1 が実際のサンプル番号なので、j+1を2で割った余りが0、
				k[i][kc++]=d[i][j];	//すなわち、偶数番の場合、訓練サンプルとしている。
			}
			else{
				t[i][tc++]=d[i][j];	// 奇数番25個をテストサンプル用配列に格納
			}
		}
		kc = 0, tc = 0;	// カウンタの初期化
	}
...
void cal_vector(double d[M][NN],double av[M]){		//引数説明:各クラスのデータ,平均ベクトル
	int s,t;
	double sum_xi[M]={0.0};
	for(t=0;t<M;t++){
		for(s=0;s<NN;s++){
...
int main(void){
  ...
  //*****************************************************************************************//
	...
	cal_vector(training_setosa,se_avector);	 		//setosaの偶数番25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_setosa,se_avector,setosa_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(setosa_CM,setosa_CMI);

	cal_vector(training_versic,ve_avector);	 		//versicolorの偶数番25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_versic,ve_avector,versic_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(versic_CM,versic_CMI);

	cal_vector(training_virgin,vi_avector);	 		//verginicaの偶数番25個のデータを訓練サンプルとして平均ベクトルを求める
