...
void holdout(double d[M][N],double k[M][NN],double t[M][NN]){
	...
	for(i=0;i<M;i++){
		for(j=0;j<N;j++){
			if(j < NN){		  // jが25よりも小さい時、すなわち前半25個までを、
				k[i][j]=d[i][j];  // 訓練サンプル用配列に格納する
			}
			else{
...
//4.共分散行列の推定	
void cal_CovarianceM(double d[M][NN],double av[M],double output[M][M]){ //引数説明: 各クラスのデータ, 平均ベクトル, 共分散出力配列
	int j,k,l;	
	double sum[M][M]={0};
...
int main(void){
  ...
  //*****************************************************************************************//
	...
	cal_vector(training_setosa,se_avector);			//setosaの前半25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_setosa,se_avector,setosa_CM);	//共分散行列を求める
	cal_inverse(setosa_CM,setosa_CMI);			//逆共分散行列を求める

	cal_vector(training_versic,ve_avector);			//versicolorの前半25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_versic,ve_avector,versic_CM);	//共分散行列を求める
	cal_inverse(versic_CM,versic_CMI);			//逆共分散行列を求める

	cal_vector(training_virgin,vi_avector);			//verginicaの前半25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_virgin,vi_avector,virgin_CM);	//共分散行列を求める
	...
	for(k = 0; k < NN; k++){
		...
		cal_distance(test,se_avector,setosa_CM,setosa_CMI,d_setosa,k);	// k番目のテストデータから、各クラスの距離を算出する
	...
	cal_vector(training_setosa,se_avector);			//setosaの前半25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_setosa,se_avector,setosa_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(setosa_CM,setosa_CMI);

	cal_vector(training_versic,ve_avector);			//versicolorの前半25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_versic,ve_avector,versic_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(versic_CM,versic_CMI);

	cal_vector(training_virgin,vi_avector);			//verginicaの前半25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_virgin,vi_avector,virgin_CM);	//共分散行列,逆共分散行列を求める
	...
	for(b=0;b<6;b++){
		...
		printf("%.1lf \t %.1lf \t %.1lf \t %.1lf\n",test_versic[0][num_c[b]] ,test_versic[1][num_c[b]] , test_versic[2][num_c[b]] , test_versic[3][num_c[b]] );
		}
	}
		printf("\n");
	...
	cal_vector(training_setosa,se_avector);			//setosaの前半25個のデータを訓練サンプルとして平均ベクトル
	cal_CovarianceM(training_setosa,se_avector,setosa_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(setosa_CM,setosa_CMI);

	cal_vector(training_versic,ve_avector);			//versicolorの前半25個のデータを訓練サンプルとして平均ベクトル
	cal_CovarianceM(training_versic,ve_avector,versic_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(versic_CM,versic_CMI);

	cal_vector(training_virgin,vi_avector);			//verginicaの前半25個のデータを訓練サンプルとして平均ベクトル
	cal_CovarianceM(training_virgin,vi_avector,virgin_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(virgin_CM,virgin_CMI);
	...
	for(b=0;b<6;b++){
		if(num_b[b]==0){}
			...
			printf("%.1lf \t %.1lf \t %.1lf \t %.1lf\n",test_virgin[0][num_b[b]] ,test_virgin[1][num_b[b]] , test_virgin[2][num_b[b]] , test_virgin[3][num_b[b]] );
		}
	}
		printf("\n");
	...
	printf("\n全テストサンプル%d個中%d個 誤識別\n",NN*O,child );	//誤識別したサンプル数の出力
	printf("誤識別率:%.3lf％\n",(double)child*100/(NN*O));		//誤識別率出力
