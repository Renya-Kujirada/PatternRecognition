...
//2.1訓練サンプル、テストサンプルをそれぞれ分割する
	/* クラス50個のサンプルのうち、後半25個を訓練サンプル、前半25個をテストサンプルとする */
	...
	for(i=0;i<M;i++){
		for(j=0;j<N;j++){
			if(j < NN){			// jが25よりも小さい時、すなわち前半25個までを、
				t[i][j]=d[i][j];	// テストサンプル用配列に格納する
			}
			else{
				k[i][j-NN]=d[i][j];	// 後半25個を訓練サンプル用配列に格納
			}
		}
	}
}
...
int main(void){
  ...
  //*****************************************************************************************//
	...
	cal_vector(training_setosa,se_avector);			 //setosaの後半25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_setosa,se_avector,setosa_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(setosa_CM,setosa_CMI);

	cal_vector(training_versic,ve_avector);	 		//versicolorの後半25個のデータを訓練サンプルとして平均ベクトルを求める
	cal_CovarianceM(training_versic,ve_avector,versic_CM);	//共分散行列,逆共分散行列を求める
	cal_inverse(versic_CM,versic_CMI);

	cal_vector(training_virgin,vi_avector);	 		//verginicaの後半25個のデータを訓練サンプルとして平均ベクトルを求める
